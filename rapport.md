# Rapport d'intégration | MonkeyStats

## Choix de structure HTML

J'ai organisé chaque page autour des balises sémantiques HTML pour que la structure reflète le sens du contenu, pas juste la mise en page.
...
**Lien d'évitement** : chaque page commence par `<a href="#contenu" class="skip-link">Aller au contenu</a>`. Il est invisible par défaut et apparaît au focus clavier (`top: 0`), qui nous renvoie a `<main>`, en soit, pour l'instant cette fonctionnalité ne sert pas a grand chose car le main est directement visible sur la page.
...
**Vérification** : j'ai testé avec l'inspecteur d'accessibilité de Firefox (arbre d'accessibilité, rôles, labels) et navigué à la souris et au clavier pour vérifier la cohérence.

---

## Répartition

**Dorian | coder** — structure HTML des trois pages (balisage sémantique, accessibilité, formulaire), CSS complet (variables, responsive, focus styles), JavaScript (raccourci clavier, copie du pseudo).

**Matteo | designer** — maquettage et choix du design (couleurs, typographie, mise en page générale), aide sur le CSS de la page classement.

**Adam | tester** — rédaction du contenu textuel, tests sur différents appareils et navigateurs.
